...
#' Create the adjustment matrix for covariates
#' @description Generate random vectors from multivariate normal distribution with mean 0 and covariance matrix P, in which \eqn P = I-1inv((t(1)1))t(1).
#' @param obj The glmmkin.randomvec object.
#' @param CovariateM The covariate matrix with first column be 1.
#' @return A list of class glmmkin.randomvec
#' \item{random.vectors}{A random matrix with dimensions equal to the sample size multiplied by \code{N.randomvec}. as.vector(r) is from multivate normal distribution with mean 0 and covariance matrix \eqn P = I-1inv((t(1)1))t(1).}
#' \item{C}{adjustment matrix of covariates}
#' @reference 
#' @author Han Chen, Nannan Wang
#' @examples
#' \donttest{
...
#' @keywords random vector
#' @export
CX <- function(null.obj, CovariateM)
{
    ...
    Ux <- t(CovariateM)%*%residuals2
    Ux <- Ux[2:nrow(Ux),]
    a_i <- solve(Ux%*%t(Ux))
    C <-t(Ux)%*%a_i%*%Ux   
    # saveRDS(C, file="random_X.RDS") 
    return(C)
}
...
#' @export
G.stat <- function(null.obj, geno.file, bgen.samplefile = NULL, meta.file.prefix, saveGDS = TRUE, N.randomvec = 1000, MAF.range = c(1e-7, 0.5), miss.cutoff = 1, missing.method = "impute2mean", nperbatch = 10000, ncores = 1)
{
    ...
    if(grepl("\\.gds$", geno.file[1])){
        ...
        if(ncores > 1) {
            ...
            foreach(b=1:ncores, .inorder=FALSE, .options.multicore = list(preschedule = FALSE, set.seed = FALSE)) %dopar% {
                ...
                write.table(t(c("SNP", "chr", "pos", "ref", "alt", "N", "missrate", "altfreq")), meta.file.sample, quote = FALSE, row.names = FALSE, col.names = FALSE)
                if (!saveGDS){
                ...
                for(i in 1:nbatch.flush) {
                    ...
                    write.table(out[,c("SNP", "chr", "pos", "ref", "alt", "N", "missrate", "altfreq")], meta.file.sample, quote=FALSE, row.names=FALSE, col.names=FALSE, append=TRUE, na=".")
                    if (!saveGDS) {
                ...
                if (!saveGDS){
                    ...
                    # RMatrix <- signif(RMatrix,digits=3)
                    # # save(RMatrix,file=paste0(meta.file.resample,".Rdata"), compress = "xz")
                    # out.fn <- paste0(meta.file.resample,".gds")
                    # savegds(RMatrix,N.randomvec,gname=out.fn)
                    # savegds(dtype="closegds", gname=out.fn)
        ...
        if(ncores > 1) {
            ...
            foreach(b=1:ncores, .inorder=FALSE, .options.multicore = list(preschedule = FALSE, set.seed = FALSE)) %dopar% {
                ...
                for(i in 1:nbatch.flush) {
                    ...
                    tmp.variant.idx <- tmp.variant.idx[include]
                    tmp.p <- length(tmp.variant.idx)
                    out<-bgenVariant$VariantInfo[tmp.variant.idx,c("RSID","CHR","POS","A1","A2"),drop=FALSE]
                    colnames(out)<-c("SNP","chr","pos","ref","alt")
                    out$missrate <- miss[include]
                    out$altfreq <- freq[include]
            ...
            for(i in 1:nbatch.flush) {
                ...
                tmp.variant.idx <- tmp.variant.idx[include]
                tmp.p <- length(tmp.variant.idx)
                out<-bgenVariant$VariantInfo[match(tmp.variant.idx,variant.idx),c("RSID","CHR","POS","A1","A2"),drop=FALSE]
                colnames(out)<-c("SNP","chr","pos","ref","alt")
                out$missrate <- miss[include]
                out$altfreq <- freq[include]
                geno <- geno[, include, drop = FALSE]
...
#a step to replace the SCORE in the output file of G.stat with regenie resutl
#and calcualte sigma2 in the function of G.pval
Score_regenie<-function(regenie_resultFile,output.prefix,ncores){
    regenie<-read.table(regenie_resultFile,header=T)
    for (i in seq(1,ncores)){
        ifile<-paste0(out.prefix,"_part",i,".freq")
        idata<-read.table(ifile,header=T)
        ID2<-intersect(idata$SNP,regenie$ID)
        idata$SCORE<-NA
        iBETA<-regenie$BETA[match(ID2,regenie$ID)]
        iSE<-regenie$SE[match(ID2,regenie$ID)]
        idata$SCORE[match(ID2,idata$SNP)]<-iBETA/(iSE)^2
        # ifile1<-paste0(ifile,"_rep")
        # system(paste("mv ",ifile, ifile1,sep=" "))
        write.table(idata,ifile,quote=F,sep="\t",row.names=F,col.names=T)
    }
    sigma2 <- mean(2*regenie$A1FREQ*(1-regenie$A1FREQ)*regenie$N*regenie$SE^2)
    return(sigma2)
}
...
G.prep <- function(meta.files.prefix, n.files = rep(1, length(meta.files.prefix)), cohort.group.idx = NULL, group.file, group.file.sep = "\t", auto.flip = FALSE)
{
    ...
    for(i in 1:n.cohort) {
        ...
        for(j in 1:n.files[i]) {
            ...
            tmp <- try(fread(paste0(meta.files.prefix[i], "_part", j,".freq"), header=TRUE, data.table = FALSE))
            if (class(tmp) == "try-error") {
                stop(paste0("Error: cannot read ", meta.files.prefix[i], "_part", j, ".freq !"))
            }
            ...
            if(auto.flip) {
                ...
                if(any(variant.idx1 & variant.idx2)) {
                    ...
                    cat("The following ambiguous variants were found in",paste0(meta.files.prefix[i], "_part.", j),":\n")
...
#' @keywords variant set-based test
#' @export
G.pval <- function(G.prep.obj, Cadj=NULL, sigma2=1, saveGDS = TRUE, MAF.range = c(1e-7, 0.5), MAF.weights.beta = c(1, 25), miss.cutoff = 1, method = "davies", tests = "E", rho = c(0, 0.1^2, 0.2^2, 0.3^2, 0.4^2, 0.5^2, 0.5, 1), use.minor.allele = FALSE)
{
    # if (!is.null(Cadj)){
    #         # Cadj<-readRDS(CadjFile)
    #     Cadj<-CX
    # }
    ...
    if (saveGDS==FALSE){
        for(i in 1:n.cohort) {
            cons[[i]] <- file(paste0(meta.files.prefix[i], "_part1.stoc"), "rb")
            N.resampling[i] <- readBin(cons[[i]], what = "integer", n = 1, size = 4)
        }
    } else {
        for(i in 1:n.cohort) {
            cons[[i]] <- paste0(meta.files.prefix[i], "_part1.stoc")
            fnamej<-paste0(cons[[i]])             
            N.resampling[i] <-readgds(fnamej,dtype="Nrandomvec")
        }
    }
                ...
                if (!saveGDS){
                    for(ij in 1:sum(tmp.include)){
                        if(U.list[[j]]$file[ij]!=current.cons[j]) {
                            ...
                            cons[[j]] <- file(paste0(meta.files.prefix[j], "_part", current.cons[j],".stoc"), "rb")
                            tmp.N.resampling <- readBin(cons[[j]], what = "integer", n = 1, size = 4)
                            if(tmp.N.resampling != N.resampling[j]) stop(paste0("Error: N.resampling in ", meta.files.prefix[j], "_part", current.cons[j], ".stoc does not match that in ",meta.files.prefix[j], "_part1.stoc"))
                            current.lines[j] <- 1
                        }
                    ...
                    jfile_num<-unique(U.list[[j]]$file)
                    for (kk in jfile_num){
                        kk_Uloc<-which(U.list[[j]]$file==kk)
                        current.cons[j] <- kk
                        cons[[j]] <- paste0(meta.files.prefix[j], "_part", current.cons[j])
                        fnamej<-paste0(cons[[j]],".stoc")
                        tmp.N.resampling <- readgds(fnamej,dtype="Nrandomvec")
                        if(tmp.N.resampling != N.resampling[j]) stop(paste0("Error: N.resampling in ", meta.files.prefix[j], "_part", current.cons[j], " does not match that in ",meta.files.prefix[j], "_part1"))
                        loc_Ugds<-U.list[[j]]$variant.idx[kk_Uloc]
